...
<script type="text/javascript" src="<%=contextPath%>/monitoringinfo/query/query.js"></script>
...
<script>
	...
	function eidtGhoa(id){
		top.$.jBox("iframe:../monitoringinfo/herb_observation_area.jsp?id="+id, {
			title:"修改常规观测区信息",
		    width: 1000,
		    height: 600,
		    top:10,
	...
	function findGhoaBySID(id){
		top.$.jBox("iframe:../monitoringinfo/herb_observation_area_details.jsp?id="+id, {
			title:"详细信息",
			top:10,
		    width: 1000,
		    height: 600,
...
<body onload="doInit();">
 ...
 <!-- <span style="width:100px;float:right;cursor: pointer;z-index: 2" onclick='toggle()'>展开/收起</span>-->
<table id="datagrid" fit="true" width="100%"></table>
	...
	<div id="toolbar">
	 ...
	 <table class='TableBlock' fit="true"  style='width:100%; margin: 0 auto; border-bottom: 0;'>
		...
		<form id="form1" name="form1" method="post"  style="display: none;">
			<table class='TableBlock' fit="true"  style='width:100%; margin: 0 auto;'>
				<tr>
					<td class='TableData' >调查日期：</td>
					<td class="TableData" colspan="">
					<select id="startYear" name="startYear" class="BigSelect" style="width:70px;">
					...
					<select id="endYear" name="endYear" class="BigSelect" style="width:70px;">
					<option value="">请选择</option>
					<%
						for(int i = 0 ;i<50;i++){
					%>
				...
				<tr>
					...
					<td class='TableData'><input type="text" class="SmallInput easyui-validatebox" validType="number[]" id='beginPlantNums'
						name='beginPlantNums' />至<input type="text" class="SmallInput easyui-validatebox" validType="number[] &beginPlantNums[]" id='endPlantNums'
						name='endPlantNums' /> </td>
				
					<td class='TableData' >草地类：</td>
					<td class='TableData'><input type="text" class='BigInput easyui-combobox' id='grassCategory'
						name='grassCategory' style="height: 28px;line-height: 28px;width:152px;"/></td>
						<td class='TableData' >草地型：</td>
					<td class='TableData'><input type="text" class='BigInput easyui-combobox' id='grassType'
						name='grassType' style="height:28px;line-height: 28px;width:172px;"/></td>
					
				</tr>
				<tr>
					<td class='TableData' >毒害草种数：</td>
					<td class='TableData' ><input type="text" class="SmallInput easyui-validatebox" validType='number[]' id='beginBadGrassNums'
						name='beginBadGrassNums' />至<input type="text" class="SmallInput easyui-validatebox" validType="number[] &beginBadGrassNums[]" id='endBadGrassNums'
						name='endBadGrassNums' /> </td>
					
					<td class='TableData' >草群平均高度：</td>
					<td class='TableData'>
						<input type="text" class="SmallInput easyui-validatebox" validType="numberBetweenLength[0,80] &addMethod[]" id='beginGrassAvgHeight' name='beginGrassAvgHeight'/>至<input type="text" class="SmallInput easyui-validatebox" validType="numberBetweenLength[0,80] &addMethod[] &beginGrassAvgHeight[]" id='endGrassAvgHeight' name='endGrassAvgHeight'/>(厘米)
					</td>
					...
					<td class='TableData'><input type="text" class="SmallInput easyui-validatebox" validType="numberBetweenLength[0,100] &addMethod[]" id='beginCoverage'
						name='beginCoverage'/>至<input type="text" class="SmallInput easyui-validatebox" validType="numberBetweenLength[0,100] &addMethod[] &beginCoverage[]" id='endCoverage'
						name='endCoverage'/>(%)</td>
				</tr>
				<tr>
					<td class='TableData' >总产草量折算：</td>
					<td class='TableData'>鲜重：<input type="text" class='SmallInput easyui-validatebox' validType="integeZerol[] &addMethodf[]" id='beginFreshAmount'
						name='beginFreshAmount'/>至<input type="text" class='SmallInput easyui-validatebox' validType="integeZerol[] &addMethodf[] &beginFreshAmount[]" id='endFreshAmount'
						name='endFreshAmount'/>(千克/公顷)</td>
					<td class='TableData' >可产草量折算：</td>
					<td class='TableData' colspan="">鲜重：<input type="text" class='SmallInput easyui-validatebox' validType="integeZerol[] &addMethodf[]" id='beginEdibleFreshAmount'
						name='beginEdibleFreshAmount'/>至<input type="text" class='SmallInput easyui-validatebox' validType="integeZerol[] &addMethodf[] &beginEdibleFreshAmount[]" id='endEdibleFreshAmount'
						name='endEdibleFreshAmount'/>(千克/公顷)</td>	
					<td class='TableData' >土壤含水量测定：</td>
					<td class='TableData'>
						<input type="text" class="SmallInput easyui-validatebox" validType="numberBetweenLength[0,100] &addMethod[]" id='beginSoilMoisture' name='beginSoilMoisture'/>至<input type="text" class="SmallInput easyui-validatebox" validType="beginSoilMoisture[] &numberBetweenLength[0,100] &addMethod[]" id='endSoilMoisture' name='endSoilMoisture'/>(%)
					</td>
				</tr>
				<tr>
					<td class='TableData'></td>
					<td class='TableData'>干重：<input type="text" class='SmallInput easyui-validatebox' validType="integeZerol[] &addMethodf[]" id='beginDryAmount'
						name='beginDryAmount'/>至<input type="text" class='SmallInput easyui-validatebox' validType="integeZerol[] &addMethodf[] &beginDryAmount[]" id='endDryAmount'
						name='endDryAmount'/>(千克/公顷)</td>
					
					<td class='TableData'></td>
					<td class='TableData' colspan="3">干重：<input type="text" class='SmallInput easyui-validatebox' validType="integeZerol[] &addMethodf[]" id='beginEdibleDryAmount'
						name='beginEdibleDryAmount'/>至<input type="text" class='SmallInput easyui-validatebox' validType="integeZerol[] &addMethodf[] &beginEdibleDryAmount[]" id='endEdibleDryAmount'
						name='endEdibleDryAmount'/>(千克/公顷)</td>
				</tr>
				...
				<tr>
					<td class="TableHeader" style="text-align:center;" colspan="6">
						...
						<input type="hidden" id="observationArea" name="observationArea" value="0" />
						...
						<input type='button' class="btn btn-success" style="background: #41a675;" value="下载"  onclick='multiExportInfo();'/>
					</td>
				</tr>
			</table>
		</form>
