...
function doInit(){
	...
	if(type=="g"){
		statistics(5);
		$("#monitoringBtn").val("添加返青期样地");
		$("#monitoringTitle").text("返青期调查");
		investigateType=5;
		typeDesc="返青期样方";
	}else if(type=="y"){
		statistics(6);
		$("#monitoringBtn").val("添加枯黄期样地");
		$("#monitoringTitle").text("枯黄期调查");
		investigateType=6;
		typeDesc="枯黄期样方";
	}
	...
	datagrid = $('#datagrid').datagrid({
		...
		columns:[[
			...
			{field:'investigateNum',title:'数据名称',width:250,height:45,rowspan:2,formatter:function(e,rowData,index){
			...
			{field:'investigate_type',title:'数据类型',width:80,sortable:true,rowspan:2,formatter:function(e,rowData,index){
				...
				if(quadratList){
					for(var i=0;i<quadratList.length;i++){
						html+="<div class='quadrat'>"+typeDesc+"</div>";
					}
				}
				return html;
			}},
		...
		[
			...
			{field:'3',title:'添加样方',width:30,align:"center",formatter:function(e,rowData,index){
				var html ="<div class='sampleArea'><a class='addCtrl' title='添加样方' href='javascript:void(0);' onclick='addQuadrat("+rowData.sampleAreaId+")'></a></div>";
				var quadratList=rowData.quadratList;
				if(quadratList){
					for(var i=0;i<quadratList.length;i++){
						html+="<div class='quadrat'>"
			...
			{field:'4',title:'修改',width:20,align:"center",formatter:function(e,rowData,index){
				...
				var html ="<div class='sampleArea'>"
						+"<a class='editCtrl' href='javascript:void(0);' title='修改' onclick=\"editGreenYellowSampleArea("+rowData.sampleAreaId+","+rowData.recordsType+",'"+auditContent+"',"+quadratList.length+")\"></a>"
						+"</div>";
				if(quadratList){
					for(var i=0;i<quadratList.length;i++){
						var content = quadratList[i].auditContent==undefined?"":quadratList[i].auditContent;
						html+="<div class='quadrat'>"
							+"<a class='editCtrl' href='javascript:void(0);' title='修改' onclick=\"editQuadrat("+quadratList[i].sid+","+quadratList[i].recordsType+",'"+content+"')\"></a>"
							+"</div>";
					}
				}
				return html;
			...
			{field:'5',title:'删除',width:20,align:"center",formatter:function(e,rowData,index){
				...
				var html ="<div class='sampleArea'><a class='deleteCtrl' title='删除' href='javascript:void(0);' onclick='deleteGreenYellowSampleArea("+rowData.sampleAreaId+","+isDel+")'></a>"
						+"</div>";
				if(quadratList){
					for(var i=0;i<quadratList.length;i++){
						html+="<div class='quadrat'>"
							+"<a class='deleteCtrl' href='javascript:void(0);' title='删除' onclick='deleteQuadrat("+quadratList[i].sid+","+quadratList[i].recordsType+")'></a>"
							+"</div>";
					}
				}
				return html;
...
function addQuadrat(sampleAreaId){
	var typeDesc="";
	if(type=="g"){
		typeDesc="添加返青期样方";
	}else if(type=="y"){
	...
	top.$.jBox("iframe:../investigation/gymonitoring/quadrat.jsp?sampleAreaId="+sampleAreaId+"&type="+type, {
		title:typeDesc,
		top:30,
		width: 800,
	    height: 550,
	    ...
	    submit: function (v, h, f) {
            if (v == 1) {
            	top.document.getElementById("jbox-iframe").contentWindow.saveGreenYellowQuadrat();
            	return false;
            }
            else if(v==2){
            	return true;
...
function editQuadrat(id,recordsType,auditContent){
	...
	top.$.jBox("iframe:../investigation/gymonitoring/quadrat.jsp?id="+id+"&type="+type, {
...
function deleteQuadrat(id,recordsType){
	if(recordsType==0 || recordsType==2){
	...
	top.$.jBox.confirm("确定删除该条样方数据？","确认",function(v){
		if(v=="ok"){
			var url = contextPath+"/gmsGreenYellowQuadratController/deleteGreenYellowQuadratById.act?sid="+id;
...
<body onload="doInit();" style="overflow: hidden; font-size: 12px">
	...
	<div id="toolbar">
		<div class="moduleHeader" style='height: 60px; padding-top: 20px;'>
			<img src='<%=contextPath%>/resource/images/sys/home.png' style='top:20px;'/><b class="first">当前位置：常规监测报送》<span id="monitoringTitle" style="color: #41a675;"></span></b>
			<div style="float: right;">
				<input type="button" class="btn btn-success" style="background: #42a576;" value="添加植被长势调查"
					id="monitoringBtn" onclick="addGreenYellowSampleArea()" /> 
				<%if(type.equals("g")) {%>
				<input type="button" class="btn btn-success" style="background: #42a576;" value="批量上报" onclick="mulitiSubmit(5)" />
				<%}else{ %>
				<input type="button" class="btn btn-success" style="background: #42a576;" value="批量上报" onclick="mulitiSubmit(6)" />
				<%} %>
			</div>
